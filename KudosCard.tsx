  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-md border-l-4 border-teal-500 flex space-x-4">
      <div className="flex-shrink-0">
        <div className={`w-14 h-14 rounded-full flex items-center justify-center ${isGold ? 'bg-yellow-100' : 'bg-gray-200'}`}>
          <StarIcon className={`w-8 h-8 ${isGold ? 'text-yellow-500' : 'text-gray-400'}`} />
        </div>
      </div>
      <div className="flex-1">
        <div className="flex justify-between items-start">
            ...
            <span className={`px-3 py-1 text-sm font-semibold rounded-full ${isGold ? 'bg-yellow-200 text-yellow-800' : 'bg-gray-200 text-gray-800'}`}>
                {kudos.type} Kudos
            </span>
        </div>
        ...
        {kudos.imageBase64 && (
          <div className="mt-3 rounded-lg overflow-hidden border">
            <img src={`data:image/png;base64,${kudos.imageBase64}`} alt="Kudos Illustration" className="w-full object-cover" />
          </div>
        )}
